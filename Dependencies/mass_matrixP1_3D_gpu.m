function M=mass_matrixP1_3D_gpu(elements,volumes,coeffs)
...
if 1
%     Xscalar=kron(int32(ones(1,4)),elements);
%     Yscalar=kron(elements,int32(ones(1,4)));
    ...
    if (nargin<3)
        Zmassmatrix=kron(volumes,gpuArray(reshape((ones(4)+eye(4))/20,1,16))); 
    else
        if numel(coeffs)==size(elements,1) %P0 coefficients
            Zmassmatrix=kron(volumes.*coeffs,gpuArray(reshape((ones(4)+eye(4))/20,1,16))); 
        else %P1 coefficients
            M1=gpuArray([6 2 2 2; 2 2 1 1; 2 1 2 1; 2 1 1 2]/120);
            M2=M1([4,1,2,3],[4,1,2,3]);
            M3=M2([4,1,2,3],[4,1,2,3]);
            M4=M3([4,1,2,3],[4,1,2,3]);

            Zmassmatrix=kron(volumes.*coeffs(elements(:,1)),gpuArray(reshape(M1,1,16))) ...
                       +kron(volumes.*coeffs(elements(:,2)),gpuArray(reshape(M2,1,16))) ...
                       +kron(volumes.*coeffs(elements(:,3)),gpuArray(reshape(M3,1,16))) ...
                       +kron(volumes.*coeffs(elements(:,4)),gpuArray(reshape(M4,1,16)));                      
        end    
    end
